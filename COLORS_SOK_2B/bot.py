...
def parse_color(value):
    ...
    if value.isdigit():
        ...
        if not result:
            raise ValueError(
                "رقم اللون يجب أن يكون من 1 إلى 100."
            )
    ...
    if not h:
        raise ValueError(
            "HEX غير صالح. مثال: #F11111 أو F11111."
        )
...
async def colors_cmd(
    ...
    if not os.path.exists(IMAGE):
        return await i.response.send_message(
            "❌ colors.png غير موجود داخل مجلد COLORS_SOK_2B.",
            ephemeral=True
        )
...
async def setcolor(
    ...
    try:
        ...
        label, hex_code = await apply_color(
            member,
            msg.content,
            i.user.display_name
        )
        ...
        await i.followup.send(
            f"✅ تم تطبيق {label} `{hex_code}` "
            f"على {member.mention}.",
            ephemeral=True
        )
        ...
        await i.followup.send(
            "❌ البوت لا يملك صلاحيات إدارة الرتب.",
            ephemeral=True
        )
...
async def on_ready():
    ...
    try:
        ...
        print(
            ...
            f"{bot.user} | commands: {len(synced)}"
        )
